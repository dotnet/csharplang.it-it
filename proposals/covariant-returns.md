...
Questo vincolo consente a un metodo di override `private` in una classe di `private` avere un tipo restituito.  Tuttavia, richiede un `public` metodo di override in `public` un tipo per avere `public` un tipo restituito.
...
> Ai fini del mapping dell'interfaccia, un membro `A` di classe corrisponde a `B` un membro di interfaccia quando:
> 
> - `A`e `B` sono metodi e gli elenchi nome, tipo e parametro formale di `A` e `B` sono identici.
> - `A`e `B` sono proprietà, il nome e il tipo `A` di `B` e sono identici `A` e hanno le stesse funzioni di `B` accesso`A` di (possono avere funzioni di accesso aggiuntive se non si tratta di un'implementazione esplicita di un membro di interfaccia).
> - `A`e `B` sono eventi e il nome e il tipo di `A` e `B` sono identici.
> - `A`e `B` sono indicizzatori, gli elenchi di parametri di tipo e `A` formale `B` di e sono identici e `A` hanno le stesse funzioni `B` di`A` accesso di (possono avere funzioni di accesso aggiuntive se non si tratta di un'implementazione esplicita di un membro di interfaccia).

viene modificato come segue:

> Ai fini del mapping dell'interfaccia, un membro `A` di classe corrisponde a `B` un membro di interfaccia quando:
> 
> - `A`e `B` sono metodi e gli elenchi di parametri di nome e formale `A` di `B` e sono identici e il tipo restituito `A` di è convertibile nel tipo restituito `B` tramite un'identità di conversione implicita dei riferimenti al tipo restituito di `B`.
> - `A`e `B` sono proprietà, il nome di `A` e `B` sono identici `A` , ha le stesse funzioni di `B` accesso`A` di (è consentito disporre di funzioni di accesso aggiuntive se non si tratta di un'implementazione esplicita di un membro `A` di interfaccia) e il tipo di `B` è convertibile nel tipo restituito tramite `A` una conversione di identità o, se è una proprietà ReadOnly, una conversione di un riferimento implicito.
> - `A`e `B` sono eventi e il nome e il tipo di `A` e `B` sono identici.
> - `A`e `B` sono indicizzatori, gli elenchi di parametri `A` formali `B` di e sono `A` identici, hanno le stesse `B` funzioni`A` di accesso di (possono avere funzioni di accesso aggiuntive se non si tratta di un'implementazione esplicita di un membro `A` di interfaccia) e il tipo di `B` è convertibile nel tipo restituito tramite `A` una conversione di identità o, se è un indicizzatore di sola lettura, una conversione implicita di riferimenti.
...
Si noti che `I3`in i metodi `I1.M()` e `I2.M()` sono stati "Uniti".  Quando si `I3`implementa, è necessario implementarli insieme.
...
Suggerisco di consentire l'implementazione di `I1.M` o `I2.M` (ma non entrambi) e considerarlo come un'implementazione di entrambi.
...
- una discussione all' <https://github.com/dotnet/roslyn/issues/357>indirizzo.

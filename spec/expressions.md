...
*  Un'espressione costituente di tipo in fase di compilazione `dynamic` viene considerata come il tipo del valore effettivo restituito dall'espressione in fase di esecuzione
...
Una ricerca di membri è il processo in base al quale viene determinato il significato di un nome nel contesto di un tipo. Una ricerca di membri può essere eseguita come parte della valutazione di un *simple_name* ([nomi semplici](expressions.md#simple-names)) o di un *member_access* ([accesso ai membri](expressions.md#member-access)) in un'espressione. Se il *simple_name* o *member_access* si verifica come *primary_expression* di un *invocation_expression* ([chiamate al metodo](expressions.md#method-invocations)), viene detto che il membro viene richiamato.

Se un membro è un metodo o un evento o se è una costante, un campo o una proprietà di un tipo delegato ([delegati](delegates.md)) o del tipo `dynamic` ([il tipo dinamico](types.md#the-dynamic-type)), il membro viene definito *richiamabile*.
...
*  Argomenti nel *argument_list* di costruttori di istanza, metodi, indicizzatori e delegati:
...
La chiamata del metodo di estensione ([chiamate al metodo di estensione](expressions.md#extension-method-invocations)) di `Select` viene elaborata riscrivendo la chiamata in una chiamata a un metodo statico:
```csharp
...
Analogamente alle chiamate di metodi generici, l'inferenza del tipo deve essere applicata anche quando un gruppo di metodi `M` contenente un metodo generico viene convertito in un tipo delegato specificato `D` ([conversioni di gruppi](conversions.md#method-group-conversions)di metodi). Dato un metodo
```csharp
Tr M<X1...Xn>(T1 x1 ... Tm xm)
```
...
*  Verifica dell'applicabilità parziale: l'applicabilità viene controllata in base al [membro della funzione applicabile](expressions.md#applicable-function-member), ignorando però i parametri i cui tipi sono sconosciuti.
...
(  )  ]  }  :  ;  ,  .  ?  ==  !=  |  ^
```
la *type_argument_list* viene quindi mantenuta come parte del *simple_name*, *member_access* o *pointer_member_access* e qualsiasi altra possibile analisi della sequenza di token viene eliminata. In caso contrario, il *type_argument_list* non è considerato parte del *simple_name*, *member_access* o *pointer_member_access*, anche se non è possibile analizzare la sequenza di token. Si noti che queste regole non vengono applicate durante l'analisi di un *type_argument_list* in una *namespace_or_type_name* ([nomi di spazio dei nomi e di tipo](basic-concepts.md#namespace-and-type-names)). L'istruzione
```csharp
F(G<A,B>(7));
```
...
Una *THIS_ACCESS* è consentita solo nel *blocco* di un costruttore di istanza, un metodo di istanza o una funzione di accesso di istanza. Ha uno dei significati seguenti:
...
Viene utilizzato un *object_creation_expression* per creare una nuova istanza di un *class_type* o di un *value_type*.
...
Il *tipo* di *object_creation_expression* deve essere un *class_type*, un *value_type* o un *type_parameter*. Il *tipo* non può essere un `abstract` *class_type*.

Il *argument_list* facoltativo ([elenchi di argomenti](expressions.md#argument-lists)) è consentito solo se il *tipo* è un *class_type* o un *struct_type*.
...
Viene utilizzato un *array_creation_expression* per creare una nuova istanza di un *array_type*.
...
Viene utilizzato un *delegate_creation_expression* per creare una nuova istanza di un *delegate_type*.
...
Viene utilizzato un *anonymous_object_creation_expression* per creare un oggetto di tipo anonimo.
...
Una *nameof_expression* viene utilizzata per ottenere il nome di un'entità di programma come stringa costante.
...
Si tratta di un caso speciale della grammatica per *null_conditional_expression* sopra. La produzione per *member_declarator* nelle [espressioni di creazione di oggetti anonimi](expressions.md#anonymous-object-creation-expressions) include quindi solo *null_conditional_member_access*.
...
Una *cast_expression* viene utilizzata per convertire in modo esplicito un'espressione in un tipo specificato.
